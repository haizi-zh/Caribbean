...
{
    list = $('[act-area="'+id+'"]');
    list_country= $('[act-area-country="'+id+'"]');
    list_city= $('[act-area-city="'+id+'"]');
    ...
    var len_country = list_country.length;
    for (var i = 0; i < len_country; i++) {
        if( $('#area_'+id)[0].checked ){ 
            list_country[i].checked = true;
        }else{
            list_country[i].checked = false;
        }
    }

    var len_city = list_city.length;
    for (var i = 0; i < len_city; i++) {
        if( $('#area_'+id)[0].checked ){ 
            list_city[i].checked = true;
        }else{
            list_city[i].checked = false;
        }
    }
}
...
{
    list = $('[act-country="'+id+'"]');
    list_city= $('[act-country-city="'+id+'"]');
    console.log(list_city);
    var len = list.length;
    for (var i = 0; i < len; i++) {
        if( $('#country_'+id)[0].checked ){ 
            list[i].checked = true;
        }else{
            list[i].checked = false;
        }
...
function confirm(){
	...
	self.location='/admin/discount/brand_add?brand_id='+id+'&shop_ids='+str;
}
function change_id(){
    target_brand_id = $("#target_brand_id")[0].value;
    select_demo(target_brand_id);
}
function select_demo(id){
	//console.log(id);
	$("#target_brand_id")[0].value=id;
    //获取城市和商店
    $.io.ajax({
        'method': 'get',
        'url': '/aj/brand/get_area_city_shop_by_brand',
        'args': {
            'brand_id': id
        },
        'onComplete': function(data){
            if(data.code == '200'){
                $('#name_list')[0].innerHTML = "";
                $('#shop_list')[0].innerHTML = data.data;
            }
        }
    })
...
function onmouseoutlike(){
    $('#name_list')[0].innerHTML = "";
}


function demo_suggest(){
	var brand_name = $("#brand_name")[0].value;
	$.io.ajax({
        'method': 'get',
        ...
        'onComplete': function(data){
            if(data.code == '200'){
			    content = '<ul style="max-height: 180px; overflow: auto;" onmouseleave="onmouseoutlike();" >';
			  	for (var i in data.data) {
			  		content += '<li onclick="select_demo('+data.data[i]['id']+');">' + data.data[i]['name'] + '   id=>' + data.data[i]['id'] + '</li>';
			  	}
			  	content+= '</ul>';
			  	$('#name_list')[0].innerHTML = content;
            }
        }
	})
}
...
$.ready(function () {
    ...
    var RICHEDITORHTML = '' +
        ...
        $('#add_discount').addEvent('click', function(){
            // if (window.uploadImgFlag) {
            //     return;
            // }
            var brand_id = $("#brand_id")[0].value;
            var shop_ids = $("#shop_ids")[0].value;
            ...
            $.io.ajax({
                'method': 'post',
                'url': '/aj/discount/add_brand_discount',
                'args': {
