...
class Mo_geography extends ZB_Model {
	...
	public function get_provinces_by_countrys($countrys){
		...
		$format_re = $this->do_country->get_provinces_by_countrys($countrys);
		#返回
		$return = array();
		foreach($format_re as $key=>$each){
			$return[$key] = $format_re[$key]->loclist['0']['zhName'];
		}
		$uni_re = array_unique($return);
        $re = array_filter($uni_re,create_function('$value','return !empty($value);'));
	...
	public function get_cities_by_provinces($countrys, $provinces){
		...
		$format_re = $this->do_city->get_cities_by_provinces($countrys, $provinces);	
		#返回
		$return = array();
        foreach($format_re as $key=>$value){
        	// $id = $value->{'_id'}->{'$id'};
			$return[$key]= $format_re[$key]->{'zhname'};
		}
		$uni_re = array_unique($return);
        $re = array_filter($uni_re,create_function('$value','return !empty($value);'));
