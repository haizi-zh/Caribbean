...
class brandtag extends ZB_Controller {
	...
	public function index(){
		...
		$list = $this->do_brandtag->get_brandtag_list();
		
		$data['list'] = $list;
		$data['offset'] = 0;
		$this->load->admin_view('admin/brandtag/list', $data);	
	}
	...
	public function edit(){
		...
		if($id){
			$info = $this->do_brandtag->get_tag_info($id);
		}
		$data['info'] = $info;
		$data['id'] = $id;
		...
		if($name){
			...
			if($id){
				...
				$add_data['type'] = 1;
				$add_data['name'] = $name;
				$re = $this->do_brandtag->update_tag($add_data, $id);
			}else{
				$add_data = array();
				$add_data['name'] = $name;
				$add_data['type'] = 1;
				$add_data['status'] = 0;
				$add_data['ctime'] = time();
				$re = $this->do_brandtag->add_tag($add_data);
			}
			echo "<script language='javascript'>alert('保存信息成功！') ;window.location.href='/admin/brandtag/' ;</script>" ;
			return;
		}

		$this->load->admin_view('admin/brandtag/edit', $data);	
	}
