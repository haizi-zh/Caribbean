<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	// zanbai.com/weixin/create_menu 
	// zanbai.com/weixin/get_menu 
	// zanbai.com/weixin/delete_menu 

class weixin extends ZB_Controller {
	CONST APPID = 'wxfdca8212108bd29a';
	CONST APPSECRET = 'a90501339d7bcdf0b611771b46e73304';
	...
	public function index(){
		...
		if($getRevEvent && is_array($getRevEvent)){
			//纽约券
		    if($getRevEvent['key'] == "MENU_KEY_MENU_CITY2"){
		    	$weObj->text($coupon_help)->reply();
		        exit;
		    }
		    // tips
		    if($getRevEvent['key'] == "MENU_KEY_MENU_REC2"){
		        $weObj->text($tips_help)->reply();
		        exit;
		    }
		    if($getRevEvent['key'] == "MENU_KEY_MENU_SHOP2"){
	...
	public function create_menu(){
		...
		$data[] = array("name"=>"攻略优惠",
						"sub_button"=>array(
						array('type'=>"click", "name"=>"热门商家", "key"=>"MENU_KEY_MENU_SHOP2"),
						array('type'=>"click", "name"=>"美国优惠券", "key"=>"MENU_KEY_MENU_CITY2"),
						array('type'=>"click", "name"=>"购物攻略", "key"=>"MENU_KEY_MENU_REC2")
						)
				 );
	...
	function add_event($type, $event, $key, $keyword, $ctime, $fromusername){
		...
		switch ($key) {
			...
			case 'https://itunes.apple.com/cn/app/zan-bai-wang-chu-jing-gou/id774874759?mt=8#':
				$type=4;
				break;
			case 'MENU_KEY_MENU_SHOP2':
				$type=5;
				break;
			case 'MENU_KEY_MENU_CITY2':
				$type=6;
				break;
			case 'MENU_KEY_MENU_REC2':
				$type=7;
				break;
