...
class Cimongo_base {
	...
	// public function switch_host($host, $dbname) {

	// 	$this->CI->config->load("cimongo");
	// 	if ( $host=='0' ) {
	// 		 $temp = $this->CI->config->item('host_list');
	// 		 $this->host_list = $temp[0];
	...
	// 		   $node_conn_str = $this->build_server_node($this->host_list, $this->dbname);
	// 		   $connection_string .= "$node_conn_str";
	// 		   $tmp_str = trim($connection_string);
	// 		   $this->connection_string = substr($tmp_str, 0, strlen($tmp_str));
	// 	}
