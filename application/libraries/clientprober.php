...
class clientprober
{
    ...
    static function get_client_agent($value = array('browser', 'platform', 'mobile'))
    {
    	if (isset($_SERVER['HTTP_USER_AGENT']))
		{
		    ...
		    clientprober::$user_agent = $_SERVER['HTTP_USER_AGENT'];
		}
        if (is_array($value))
        {
            $agent = array();
            foreach ($value as $v)
            {
                ...
                $agent[$v] = clientprober::get_client_agent($v);
            }
        ...
        $user_agent_conf = require dirname(__FILE__) . DIRECTORY_SEPARATOR . 'user_agents.php';
        if ($value === 'browser' OR $value == 'version')
        {
            ...
            {
                if (stripos(clientprober::$user_agent, $search) !== FALSE)
                {
                    // Set the browser name
                    $info['browser'] = $name;

                    if (preg_match('#'.preg_quote($search).'[^0-9.]*+([0-9.][0-9.a-z]*)#i', clientprober::$user_agent, $matches))
                    {
