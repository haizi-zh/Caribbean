...
class Wechat
{
	...
	*/
	...
	public function createMenu($data){
		...
		//
		$result = $this->http_post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token, self::json_encode($data) );var_dump($result);die;
		if ($result)
		{
			$json = json_decode($result,true);
	...
	public function getMenu(){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_get(self::API_URL_PREFIX.self::MENU_GET_URL.'access_token='.$this->access_token);var_dump($result);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || isset($json['errcode'])) {
